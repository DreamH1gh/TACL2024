...
from models.topk_chy import HierVerbPromptForClassification
...
def main():
    ...
    if args.eval_full:
        ...
        # embedding_list = pickle.load(open(str(args.dataset)+'_pkl/_'+str(args.shot)+'shot_none_'+str(args.seed)+'_embed_doc_' + str(args.label_description) + '.pkl',"rb"))
        embedding_list = pickle.load(open('_'+str(args.shot)+'shot_none_'+str(args.seed)+'_embed_doc_' + str(args.label_description) + '.pkl',"rb"))
        embedding_store = {'embedding':[], 'label':embedding_list['label']}
        for i in range(len(embedding_list['embedding'])):
            # embedding_store['embedding'].append(torch.Tensor(embedding_list['embedding'][i][0]+embedding_list['embedding'][i][1]))
            embedding_store['embedding'].append(torch.Tensor(embedding_list['embedding'][i][1]))
            # print(embedding_store['embedding'][0].shape)
            # exit(0)
        embedding_store['embedding'] = torch.stack(embedding_store['embedding'], dim=0).to(device)
        scores, similar_samples = prompt_model.evaluate(test_dataloader, processor, embedding_store, topk=args.topk, desc="test", mode=args.eval_mode)
        macro_f1_1 = scores['macro_f1']
        micro_f1_1 = scores['micro_f1']
        print_info('finally best macro {} micro {}'.format(macro_f1_1, micro_f1_1))
        for i in range(len(similar_samples['idx'])):
            similar_samples['context'].append([])
            for j in range(len(similar_samples['idx'][i])):
                similar_samples['context'][-1].append(train_data[similar_samples['idx'][i][j]][0])
        pickle.dump(similar_samples, open(str(args.dataset)+'_similar_samples/_'+str(args.shot)+'shot_none_'+str(args.seed)+'_' + str(args.label_description) + '.pkl',"wb"))
        # # for best micro
